...
if(!User::isLoggedIn()) { Url::redirect('index.php?p=admin/login'); }
...
if(!isset($image['meta']->username) || __S('user-username') !== $image['meta']->username) { die("Dont try this!"); }
...
if (isset($image['output']['data'])) {
	...
	$parts = explode(".", $image['output']['name']);
	$name = $image['meta']->username . "." . $parts[count($parts)-1];
    ...
    $output = Slim::saveFile($data, $name, "../templates/images/user_images/", false);
}
...
if (isset($image['input']['data'])) {
    ...
    // If you want to prevent Slim from adding a unique id to the file name add false as the fourth parameter.
    // $file = Slim::saveFile($data, $name, 'tmp/', false);
    $input = Slim::saveFile($data, $name, "../templates/images/user_images/", false);
...
//save the image name to the users field in the database
User::updateUsersIcon();
